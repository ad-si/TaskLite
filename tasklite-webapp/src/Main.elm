...
type Msg
  = ReceivedTime Posix
  | ReloadTasks
  | GotTasksResponse (RemoteData (Graphql.Http.Error (List TodoItem)) (List TodoItem))
  | NewTask String
  | AddTaskNow
  | AddTaskAt Posix
  | AddTask Posix Ulid
  ...
  | LoadMoreTasks
  | GotMoreTasksResponse (RemoteData (Graphql.Http.Error (List TodoItem)) (List TodoItem))
  | UrlChanged Url
...
type alias Model =
  { key : Key
  , remoteTodos : RemoteData (Graphql.Http.Error (List TodoItem)) (List TodoItem)
  , newTask : String
        ...
        , span
            [ css
                ...
                , case todo.due_utc of
                  Just due_utc ->
                    if due_utc < Iso8601.fromTime now
                      then Css.batch
                        [ text_color red_600
                        , dark [ text_color red_400 ]
                        ]
                      else text_color inherit
                  Nothing ->
                    text_color inherit
                ]
            ]
  ...
  div
    ...
    [ main_
        ...
        , let
            inputForm =
              form
                [ onSubmit AddTaskNow, css [ flex, mb_3 ] ]
                [ input
                    [ type_ "text"
...
insertTodo now ulid body =
  ...
  Mutation.insert_tasks
    identity
    { objects = [ buildTasks_insert_input
          { ulid = ulidString
          , body = body
...
update msg model =
  case msg of
    ...
    AddTask time ulid ->
      ( { model | submissionStatus = RemoteData.Loading }
      , if model.newTask /= ""
          then insertTodo time ulid model.newTask
          else Cmd.none
      )
    NewTask body ->
